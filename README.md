...
├── logo.png     → logo do açougue
└── README.md
```

Estrutura plana (sem subpastas) para funcionar corretamente no GitHub Pages.
...
- Endereço, horário de funcionamento e WhatsApp já estão preenchidos com os dados reais do açougue (em `index.html` e no número de checkout do `script.js`).

## O que ainda falta ajustar antes de publicar de verdade

1. **Produtos e preços** — em `script.js`, no objeto `PRODUCTS`, cada corte tem `name`, `desc`, `price` e `unit`. Edite, adicione ou remova itens livremente nas três categorias (`bovinos`, `aves`, `suinos`).
...
As cores ficam no topo do `styles.css`, dentro de `:root`:
